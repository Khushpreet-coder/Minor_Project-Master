...
// ==================== WORKOUTS (with category) ====================

// Add a workout with category
app.post('/workouts', (req, res) => {
  const { title, details, category } = req.body;
  if (!title || !details || !category) {
    return res.status(400).json({ success:false, message:'Missing fields' });
  }
  const sql = 'INSERT INTO workouts (title, details, category) VALUES (?, ?, ?)';
  db.query(sql, [title, details, category], err => {
    if (err) return res.status(500).json({ success:false, message:'Failed to add workout' });
    res.status(201).json({ success:true, message:'Workout added successfully' });
  });
});

// Fetch workouts, optionally filtered by ?category=underweight|normal|overweight
app.get('/workouts', (req, res) => {
...
app.post('/nutrition', (req, res) => {
  ...
  const sql = 'INSERT INTO nutrition_plans (title, image_url, category) VALUES (?, ?, ?)';
