...
//       const input = prompt('Enter Nutrition Title, Image URL, and Category (comma-separated):');
//       if (!input) return alert('Input required!');
//       const [title, image_url, category] = input.split(',').map(item => item.trim());
//       if (!title || !image_url || !category) return alert('All fields are required!');
